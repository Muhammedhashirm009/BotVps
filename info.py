...
BOT_TOKEN = "5858563808:AAFRnnYQdomkB_NxOvyHUEJXWL2c_GLYWlQ"
...
ADMINS = [int(admin) if id_pattern.search(admin) else admin for admin in environ.get('ADMINS', '2139560486').split()]
CHANNELS = [int(ch) if id_pattern.search(ch) else ch for ch in environ.get('CHANNELS', '-1001704871970').split()]
auth_users = [int(user) if id_pattern.search(user) else user for user in environ.get('AUTH_USERS', '2139560486').split()]
AUTH_USERS = (auth_users + ADMINS) if auth_users else []
auth_channel = "-1001704871970"
auth_grp = "-1001704871970"
AUTH_CHANNEL = int(auth_channel) if auth_channel and id_pattern.search(auth_channel) else None
AUTH_GROUPS = [int(ch) for ch in auth_grp.split()] if auth_grp else None

# MongoDB information
DATABASE_URL = "mongodb+srv://hashir:gUeHLPQ5i7ARzwkA@cluster0.rvkb8jf.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0"
